             ...
             int const warning, int const critical, int const decimals)
{
  if (critical != 0 && used > critical) {
    printf("<span font='FontAwesome'>%s</span><span color='%s'>", label, RED);
  } else if (warning != 0 && used > warning) {
    printf("<span font='FontAwesome'>%s</span><span color='%s'>", label, ORANGE);
  } else {
    printf("<span font='FontAwesome'>%s</span><span>", label);
  }
